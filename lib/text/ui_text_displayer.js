  ...
  setCaptionStyles_(cueElement, cue, parents, hasWrapper) {
    ...
    if (cue.line != null) {
      ...
      if (lineInterpretation == Cue.lineInterpretation.PERCENTAGE) {
        ...
        if (cue.writingMode == Cue.writingMode.HORIZONTAL_TOP_TO_BOTTOM) {
          style.width = '100%';
          // Assuming about 11 horizontal lines per screen
          if (cue.line >= 0) {
            style.top = (cue.line * 9) + '%';
          } else {
            style.bottom = (-cue.line * 9) + '%';
          }
        } else if (cue.writingMode == Cue.writingMode.VERTICAL_LEFT_TO_RIGHT) {
          style.height = '100%';
          // Assuming about 25 vertical lines per screen
          if (cue.line >= 0) {
            style.left = (cue.line * 4) + '%';
          } else {
            style.right = (-cue.line * 4) + '%';
          }
        } else { // Cue.writingMode.VERTICAL_RIGHT_TO_LEFT
          style.height = '100%';
          if (cue.line >= 0) {
            style.right = (cue.line * 4) + '%';
          } else {
