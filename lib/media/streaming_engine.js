...
shaka.media.StreamingEngine = class {
  ...
  async onUpdate_(mediaState) {
    ...
    if (!mediaState.stream.segmentIndex) {
      shaka.log.info('wli', 'creating segment index', mediaState.type);
  ...
  update_(mediaState) {
    ...
    shaka.log.info('wli', 'update_ bufferedAhead', bufferedAhead);
    ...
    if (!reference) {
      shaka.log.info('wli', 'update_ no reference yet');
    ...
    if (mediaState.segmentPrefetch && mediaState.segmentIterator) {
      shaka.log.info('wli', 'update_ do prefetch');
  ...
  getSegmentReferenceNeeded_(mediaState, presentationTime, bufferEnd) {
    ...
    shaka.log.info('wli', 'getSegmentReferenceNeeded_ 1');
    if (mediaState.segmentIterator) {
      ...
      // appended.
      shaka.log.info('wli', 'getSegmentReferenceNeeded_ 2',
      ...
      if (inaccurateTolerance) {
        ...
        ref = mediaState.segmentIterator &&
            mediaState.segmentIterator.next().value;
        shaka.log.info('wli', 'getSegmentReferenceNeeded_ inaccurateTolerance');
      }
      if (!ref) {
        ...
        ref = mediaState.segmentIterator &&
            mediaState.segmentIterator.next().value;
        shaka.log.info('wli', 'getSegmentReferenceNeeded_ getIteratorForTime',
            presentationTime);
      }
      if (ref == null) {
  ...
  async fetchAndAppend_(mediaState, presentationTime, reference) {
    ...
    if (reference.initSegmentReference &&
        reference.initSegmentReference.segmentData) {
      shaka.log.info('wli', 'fetchAndAppend_ segmentData',
          reference.initSegmentReference.segmentData.byteLength);
    }
  ...
  async initSourceBuffer_(mediaState, reference) {
    ...
    if (!shaka.media.InitSegmentReference.equal(
      ...
      if (reference.initSegmentReference) {
        ...
        shaka.log.info('wli', reference.initSegmentReference.getUris(),
            reference.initSegmentReference.startByte,
            reference.initSegmentReference.endByte,
            reference.initSegmentReference.mediaQuality);
        if (reference.initSegmentReference.segmentData) {
          shaka.log.info('wli', 'segmentData',
              reference.initSegmentReference.segmentData.byteLength);
        }
        ...
        const append = async () => {
          try {
            ...
            this.destroyer_.ensureNotDestroyed();
            shaka.log.info('wli', 'initSegment', initSegment.byteLength);
            ...
            shaka.log.info('wli', 'beforeAppendSegment returned');
            ...
            shaka.log.info('wli', 'mediaSourceEngine.appendBuffer returned');
          } catch (error) {
